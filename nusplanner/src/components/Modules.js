...
function Modules () {
  ...
  const getMod = () => {
    ...
    if (code.length === 0) {
      ...
      result = result.filter((mod) => {
        if (
          mod.moduleCode.slice(0, code.length) ===
          code.toUpperCase() /* || mod.title.toUpperCase().includes(code.toUpperCase()) */
        ) {
          return true
        } else {
          return false
  ...
  const getExamMod = (array) => {
    if (hasExam) {
      return array.filter(
        (indivMod) =>
          indivMod.semesterData.filter((semDetails) => 'examDate' in semDetails)
            .length > 0
      )
    } else {
      return array
  ...
  const getHasSem1 = (array) => {
    if (hasSem1) {
      return array.filter(
        (indivMod) =>
          indivMod.semesterData.filter((semDetails) => {
  ...
  const zeroToThree = (array) => {
    if (hasZeroToThree) {
      return array.filter((indivMod) => {
        const modVal = parseInt(indivMod.moduleCredit)
  ...
  const multipleSemsSelected = () => {
    const check = [hasSem1, hasSem2, hasSem3, hasSem4]
  ...
  const handleMultipleSemFilter = (currVal) => {
    const check = [hasSem1, hasSem2, hasSem3, hasSem4]
    const allFilters = [getHasSem1, getHasSem2, getHasSem3, getHasSem4]
    let finalOutput = []
  ...
  const multipleMcSelected = () => {
    const check = [hasZeroToThree, hasFour, hasFiveToEight, hasMoreThanEight]
  ...
  const handleMultipleMcFilter = (currVal) => {
    const check = [hasZeroToThree, hasFour, hasFiveToEight, hasMoreThanEight]
    const allFilters = [zeroToThree, four, fiveToEight, moreThanEight]
    let finalOutput = []
    for (let i = 0; i < check.length; i++) {
      if (check[i] === true) {
        const currentFilter = allFilters[i]
                  ...
                  <div key={index}>
                    <Card>
                      <Card.Body>
                        ...
                        <b>
                          <a
                            href={'https://nusmods.com/modules/' + m.moduleCode}
                            target="_blank"
                            rel="noreferrer"
                          >
                            Link to NUSMODS page
                          </a>
                        </b>
                      </Card.Body>
                    </Card>
