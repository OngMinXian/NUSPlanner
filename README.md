...
We constantly run the code on the deployment server and ensure that there are no errors before submitting pushing the code to GitHub. Branching is being used to implement new features and pull requests are issued each time a merge is initiated. Adding succinct commit messages on GitHub enable us to understand all changes made to the code and ensure the consistency of our code on all levels.
